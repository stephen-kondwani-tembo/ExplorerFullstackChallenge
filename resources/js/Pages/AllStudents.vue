<template>
  <Layout>
    <div>
      <div style="height: 40px"></div>

      <div v-if="hideStudentsTable" class="container">
        <button
          @click="toogleFormShow"
          style="border-radius: 10px"
          class="btn orange waves-effect"
        >
          add student
        </button>

        <div style="height: 20px"></div>

        <table
          v-if="students.length > 0"
          class="card centered z-depth-3 responsive"
          style="border-radius: 20px"
        >
          <thead>
            <tr>
              <th>#</th>
              <th>Name</th>
              <th>Email</th>
              <th>Phone</th>
              <th>Date</th>
              <th></th>
            </tr>
          </thead>

          <tbody>
            <tr v-for="(student, index) in students" :key="index">
              <td>
                <b>{{ ++index }}.</b>
              </td>
              <td>{{ student.first_name }} {{ student.last_name }}</td>
              <td>{{ student.email }}</td>
              <td>{{ student.phone }}</td>
              <td>{{ student.created_at }}</td>
              <td>
                <Link :href="`/Student/${student.id}`">
                  <button class="btn btn-floating white">
                    <i class="material-icons black-text">remove_red_eye</i>
                  </button>
                </Link>

                <Link :href="`/EditStudent/${student.id}`">
                  <button class="btn btn-floating white">
                    <i class="material-icons black-text">edit</i>
                  </button>
                  </Link>

                <a
                  @click="deleteStudent(student.id)"
                  class="btn btn-floating white"
                >
                  <i class="material-icons black-text">delete</i>
                </a>
              </td>
            </tr>
          </tbody>
        </table>
        <div v-else>
          <h3 class="center grey-text">No Students Available</h3>
          <div style="height: 50px"></div>
        </div>
      </div>

      <div v-else>
        <AddStudent @toogleFormHideEvent="toogleFormHide($event)" />
      </div>

      <div style="height: 20px"></div>
    </div>
  </Layout>
</template>

<script>
import Vue from "vue";
import axios from "axios";
import Layout from "../Shared/Layout.vue";
import AddStudent from "./AddStudent.vue";
import { Link } from "@inertiajs/inertia-vue";
import VueToast from "vue-toast-notification";
import "vue-toast-notification/dist/theme-sugar.css";

Vue.use(VueToast);

export default {
  name: "AllStudents",

  components: {
    Layout,
    Link,
    AddStudent,
  },

  data() {
    return {
      students: Array,
      hideStudentsTable: true,
    };
  },

  methods: {
    deleteStudent(id) {
      axios
        .delete(`api/student/${id}`)
        .then((response) => {
          this.getStudents();
          Vue.$toast.success("Student deleted successfully.", {
            position: "top-right",
          });
        })
        .catch((error) => {
          console.log(error);
        });
    },

    getStudents() {
      axios
        .get("api/students")
        .then((response) => {
          this.students = response.data;
        })
        .catch((err) => {
          console.log();
        });
    },

    toogleFormShow() {
      this.hideStudentsTable = false;
    },

    toogleFormHide() {
      this.getStudents();
      this.hideStudentsTable = true;
    },
  },

  created() {
    axios
      .get("api/students")
      .then((response) => {
        this.students = response.data;
      })
      .catch((err) => {
        console.log();
      });
  },
};
</script>