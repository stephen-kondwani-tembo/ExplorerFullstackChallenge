...
<Layout>
  <div>
    ...
    <div class="container">
       <Link href="/AllTutors"><button class="btn btn-floating red"><i class="material-icons">arrow_back</i></button></Link>
    </div>
...
export default {
  ...
  data() {
    return {
      form: { first_name: this.tutor.first_name, last_name: this.tutor.last_name, phone: this.tutor.phone, email: this.tutor.email },
    };
  },
  ...
  methods: {
    submit() {
      axios
        .put(`http://localhost:8000/api/tutor/${this.tutor.id}`, this.form)
        .then(() => {
          ...
          Vue.$toast.success("Tutor updated successfully.", {
            position: "top-right",
          });
          this.$emit("toogleEditFormHideEvent");
        })
        .catch((error) => {
          Vue.$toast.success("something went wrong");
        });
    },
